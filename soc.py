...
def real_time():
    ...
    obstacle = obstacleDetecteur()
    lane = laneDetecteur()
    server_ip = "172.31.71.164"
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    server_port = 8080
    try:
        while True:
            frame = camera.capture_array()
            clss_panneaux = panneaux.predict_real_time(frame)
            angle = lane.predict_angle_realtime(frame)
            distance = obstacle.predict_real_time(frame)
            message  = Message(angle, clss_panneaux, distance)
